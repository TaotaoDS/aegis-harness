...
export function CEOTab({ initial, onSave }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-white mb-1">{t.ceo.title}</h2>
      ...
      <div>
        ...
        <input
          ...
          placeholder={t.ceo.agentNamePlaceholder}
          maxLength={40}
        />
      ...
      <div className="bg-slate-900/50 border border-slate-700 rounded-xl p-4">
        <div className="text-xs text-slate-500 mb-2 uppercase tracking-wide">{t.ceo.preview}</div>
        <div className="flex items-start gap-3">
          ...
          <div className="bg-slate-800 rounded-xl rounded-tl-none px-4 py-3 max-w-xs">
            ...
            <p className="text-slate-300 text-sm">
              {t.ceo.previewGreeting(form.agent_name || "CEO Agent")}
            </p>
          </div>
        </div>
      </div>
      ...
      >
        {saving ? t.ceo.saving : saved ? t.ceo.saved : t.ceo.saveBtn}
      </button>
    </div>
  );
}
